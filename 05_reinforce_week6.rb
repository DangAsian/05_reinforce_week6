...
isHosted: false
  ...
  my_hash[:response][:results].each do |article|
    if article.has_value?("article")
        article[:views] = 0
    end
  end

  my_array = my_hash[:response][:results]
  ...
  def read_article(array)
    index = array.length
    random_article = rand(0..index - 1)
    puts array[random_article][:views] += 1

  end



  def display_article(array)
    array.each do |article|
    puts article[:webTitle]
    puts "Amount of viewers = #{article[:views]}"

  end
  end

  50.times do
  read_article(my_array)
  end
  # puts my_arr
   display_article(my_array)
